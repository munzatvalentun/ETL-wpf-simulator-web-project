...
Після застосування міграцій у БД немає жодного користувача. Зареєструйте першого через веб-інтерфейс, а потім вручну підвищте роль до `Admin` у SSMS або через будь-який SQL-клієнт:
...
Або через Visual Studio: встановіть `ETL_web_project` як стартовий проєкт, натисніть **F5**.
...
Або запустіть `ETL_simulator.exe` з папки `bin/Debug/net8.0-windows/`.

> **Порядок важливий:** WPF-додаток можна запускати до або після веб-додатку — вони незалежні. Але кнопка **"Run Job"** у веб-інтерфейсі спрацює лише якщо WPF запущений.
...
# Або окремо
dotnet test ETL_simulator.Tests/ETL_simulator.Tests.csproj
dotnet test ETL_web_project.Tests/ETL_web_project.Tests.csproj
```
